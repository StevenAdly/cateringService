		...
		fullScreen: function(){
			if( $fullScreenEl.length > 0 ) {
				$fullScreenEl.each( function(){
					...
					if( element.attr('id') == 'slider' && !element.hasClass('steven-slider-grid') ) { if( element.has('.swiper-slide') ) { element.find('.swiper-slide').css('height', scrHeight); } }
				});
			}
		},
	...
	SEMICOLON.header = {
		...
		topsearch: function(){
			...
			$(document).on('click', function(event) {
				...
				if (!$(event.target).closest('#primary-menu.mobile-menu-off-steven > ul').length) { $('#primary-menu.mobile-menu-off-steven > ul').toggleClass('show', false); }
				if (!$(event.target).closest('#primary-menu.mobile-menu-off-steven > div > ul').length) { $('#primary-menu.mobile-menu-off-steven > div > ul').toggleClass('show', false); }
			});
